      ...
      >
        ...
        <div className='col-start-1 row-start-2 overflow-hidden rounded-xl bg-neutral-100 sm:col-span-5 sm:row-span-full sm:rounded-3xl'>
          <Image
            ...
            src={process.env.NEXT_PUBLIC_API_URL + feedback?.author?.img}
          />
        </div>
        <figcaption className='text-sm text-neutral-950 sm:col-span-7 sm:row-start-3 sm:text-base'>
          <span className='font-semibold'>{feedback?.author?.name}</span>
          <span>, </span>
          <span className='sm:font-semibold'>{feedback?.author?.jobTitle}</span>
        </figcaption>
      </motion.figure>
    </motion.div>
  );
